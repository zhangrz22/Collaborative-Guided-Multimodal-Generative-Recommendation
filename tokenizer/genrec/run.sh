#!/bin/bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

INPUT_FILE=${INPUT_FILE:-/llm-reco-ssd-share/zhangrongzhou/Collaborative-Guided-Multimodal-Generative-Recommendation/data/item_text_embeddings.parquet}
OUTPUT_FILE=${OUTPUT_FILE:-/llm-reco-ssd-share/zhangrongzhou/Collaborative-Guided-Multimodal-Generative-Recommendation/data/tiger_data/Beauty/item_genrec_codes.parquet}
MODEL_PATH=${MODEL_PATH:-/llm-reco-ssd-share/zhangrongzhou/Graduation_project/models/beauty_genrec_rqvae.pth}

# Model
N_E_LIST=${N_E_LIST:-"256 256 256 256"}
E_DIM=${E_DIM:-32}
ENCODER_DIMS=${ENCODER_DIMS:-"1024 512 256 128"}
COMMITMENT_WEIGHT=${COMMITMENT_WEIGHT:-0.25}
QUANT_LOSS_WEIGHT=${QUANT_LOSS_WEIGHT:-1.0}
SK_EPSILON=${SK_EPSILON:-0.003}
SK_ITERS=${SK_ITERS:-100}

# Training
EPOCHS=${EPOCHS:-200}
BATCH_SIZE=${BATCH_SIZE:-1024}
LR=${LR:-1e-3}
WEIGHT_DECAY=${WEIGHT_DECAY:-1e-4}
GUMBEL_TEMP=${GUMBEL_TEMP:-0.2}
NUM_WORKERS=${NUM_WORKERS:-4}
SEED=${SEED:-42}

# Refine
MAX_REFINE_ROUNDS=${MAX_REFINE_ROUNDS:-20}
TARGET_COLLISION_RATE=${TARGET_COLLISION_RATE:-0.0}

LOG_FILE="${SCRIPT_DIR}/genrec_rqvae_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "$OUTPUT_FILE")" "$(dirname "$MODEL_PATH")"

echo "GenRec RQ-VAE tokenizer"
echo "  input=${INPUT_FILE}"
echo "  output=${OUTPUT_FILE}"
echo "  model=${MODEL_PATH}"
echo "  n_e_list=${N_E_LIST}, e_dim=${E_DIM}, encoder_dims=${ENCODER_DIMS}"
echo "  epochs=${EPOCHS}, batch=${BATCH_SIZE}, lr=${LR}, gumbel_t=${GUMBEL_TEMP}"

nohup python3 "${SCRIPT_DIR}/process_embedding.py" \
  --input_file "${INPUT_FILE}" \
  --output_file "${OUTPUT_FILE}" \
  --model_path "${MODEL_PATH}" \
  --n_e_list ${N_E_LIST} \
  --e_dim "${E_DIM}" \
  --encoder_dims ${ENCODER_DIMS} \
  --commitment_weight "${COMMITMENT_WEIGHT}" \
  --quant_loss_weight "${QUANT_LOSS_WEIGHT}" \
  --sk_epsilon "${SK_EPSILON}" \
  --sk_iters "${SK_ITERS}" \
  --epochs "${EPOCHS}" \
  --batch_size "${BATCH_SIZE}" \
  --lr "${LR}" \
  --weight_decay "${WEIGHT_DECAY}" \
  --gumbel_temp "${GUMBEL_TEMP}" \
  --num_workers "${NUM_WORKERS}" \
  --seed "${SEED}" \
  --max_refine_rounds "${MAX_REFINE_ROUNDS}" \
  --target_collision_rate "${TARGET_COLLISION_RATE}" \
  > "${LOG_FILE}" 2>&1 &

PID=$!
echo "PID=${PID}"
echo "Log: ${LOG_FILE}"
echo "Watch: tail -f ${LOG_FILE}"

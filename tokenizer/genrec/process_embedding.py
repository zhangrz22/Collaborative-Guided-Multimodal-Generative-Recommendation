#!/usr/bin/env python3
"""
GenRec-style RQ-VAE tokenizer for item embeddings.
...
def refine_collisions(model, emb, codes, device, max_rounds=20, target_rate=0.0):
    """Re-encode collision groups with Sinkhorn on last layer."""
    ...
    # Force Sinkhorn on last layer for refine
    saved_sk = []
    for vq in model.rq.vq_layers:
        saved_sk.append(vq.use_sinkhorn)
        vq.use_sinkhorn = False
    model.rq.vq_layers[-1].use_sinkhorn = True
    ...
    for r in range(1, max_rounds + 1):
        ...
        if rate <= target_rate or not collision_groups:
            break
        print(f"[Refine] round={r} rate={rate:.4f}, groups={len(collision_groups)}, max_dup={max_dup}")

        for group in collision_groups:
            t = torch.from_numpy(emb[np.array(group)]).to(device)
            # Use model.rq forward in eval mode (argmin), but with Sinkhorn
            # We need to do manual encode with Sinkhorn on last layer
            z = model.encoder(t)
            residual = z
            new_ids = []
            for vq in model.rq.vq_layers:
                dist = vq._distances(residual)
                if vq.use_sinkhorn and vq.sk_epsilon > 0 and dist.shape[0] > 1:
                    max_d, min_d = dist.max(), dist.min()
                    mid = (max_d + min_d) / 2
                    amp = (max_d - mid).clamp(min=1e-5)
                    from rq_vae import sinkhorn_knopp
                    cost_norm = ((dist - mid) / amp).double()
                    P = sinkhorn_knopp(cost_norm, eps=vq.sk_epsilon, max_iter=vq.sk_iters)
                    idx = P.argmax(dim=-1).long()
                else:
                    idx = dist.argmin(dim=1)
                emb_q = vq.embedding(idx)
                residual = residual - emb_q
                new_ids.append(idx)
            new_codes = torch.stack(new_ids, dim=1).cpu().numpy()
            for j, item_idx in enumerate(group):
                codes[item_idx] = new_codes[j]
    ...
    # Restore
    for vq, s in zip(model.rq.vq_layers, saved_sk):
        vq.use_sinkhorn = s
...
def train(model, emb, args, device):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        n = 0
        n_layers = len(args.n_e_list)
        code_counts = torch.zeros(n_layers, max(args.n_e_list), dtype=torch.long)

        for (batch_x,) in tqdm(loader, desc=f"Epoch {epoch}/{args.epochs}"):
            batch_x = batch_x.to(device, non_blocking=True)
            optimizer.zero_grad(set_to_none=True)
            out = model(batch_x, gumbel_t=gumbel_t)
            out["loss"].backward()
            ...
            for layer in range(codes.shape[1]):
                code_counts[layer] += torch.bincount(
                    codes[:, layer], minlength=args.n_e_list[layer]
                ).cpu()
        ...
        print(f"[Epoch {epoch}] loss={epoch_loss:.4f} recon={total_recon/n:.4f} "
              f"vq={total_vq/n:.4f} | {' '.join(usage_parts)}")

        if epoch_loss < best_loss:
            best_loss = epoch_loss
            save_checkpoint(model, args.model_path, args)
            print(f"  -> saved (loss={epoch_loss:.4f})")
...
def parse_args():
    ...
    p.add_argument("--gumbel_temp", type=float, default=0.2)
